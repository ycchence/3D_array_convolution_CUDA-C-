...
__global__ void vector_max_kernel3(float *in, float *out, int N) {
    ...
    if (threadIdx.x == 0) {
        ...
        while((end=end/2)>1) {
            for (int i = 0 ; i<end ;i++)
            {
                if(share[tid + i] < share[tid + i + end])
                     share[tid+i] = share[tid + i + end];
            }
    ...
    if(tid<N){ 
            for(int i=0; nn>0; i++){
                if (tid%a==0){
                    ...
                    if(share[tid]<share[tid+(a/2)])
                        share[tid]=share[tid+(a/2)];
                }
                if (threadIdx.x == 0){
                    a=a*2;
                    nn = nn/2;    
...
__global__ void vector_max_kernel4(float *in, float *out, int N) {
   ...
   // int  dex=4, st=2;
    ...
    if(tid<N){
       ...
       if(tid<ln[tid])
       {
           if(share[tid]<share[tid+ln[tid]])
              share[tid]=share[tid+ln[tid]];           
       }
       __syncthreads();
       while(ln[tid]>0){
         if(tid<ln[tid]){
...
float GPU_vector_max(float *in_CPU, int N, int kernel_code, float *kernel_runtime, float *transfer_runtime) {
    ...
    float *in_GPU, *out_GPU, *temp, *temp2;
    ...
    if (cudaMalloc((void **) &temp, vector_size) != cudaSuccess) die("Error allocating GPU memory");
    if (cudaMalloc((void **) &temp2, vector_size) != cudaSuccess) die("Error allocating GPU memory");
    ...
    case 1 : 
       if(num3>256){
           vector_max_kernel <<< grid_size , threads_per_block >>> (in_GPU, temp, num3);
           num3 = num3 / 256;
           j=0;
           for (int i = 0; num3>256; i++){
               if (j==0){
                    vector_max_kernel <<< grid_size , threads_per_block >>> (temp, temp2, num3);
                    j=1;
               }
               if (j==1){
                    vector_max_kernel <<< grid_size , threads_per_block >>> (temp2, temp, num3);
                    j=0;
                } 
                num3 = num3 / 256;
           }
           if (j==0)
                vector_max_kernel <<< grid_size , threads_per_block >>> (temp, out_GPU, num3);
           if (j==1)
                vector_max_kernel <<< grid_size , threads_per_block >>> (temp2, out_GPU, num3); 
        }
        else 
           vector_max_kernel <<< grid_size , threads_per_block >>> (in_GPU, out_GPU, num3);
    
        break;

    case 2 :
       if(num3>256){
           vector_max_kernel2 <<< grid_size , threads_per_block >>> (in_GPU, temp, num3);
           num3 = num3 / 256;
           j=0;
           for (int i = 0; num3>256; i++){
               if (j==0){
                    vector_max_kernel2 <<< grid_size , threads_per_block >>> (temp, temp2, num3);
                    j=1;
               }
               if (j==1){
                    vector_max_kernel2 <<< grid_size , threads_per_block >>> (temp2, temp, num3);
                    j=0;
                }
                num3 = num3 / 256;
           }
           if (j==0)
                vector_max_kernel2 <<< grid_size , threads_per_block >>> (temp, out_GPU, num3);
           if (j==1)
                vector_max_kernel2 <<< grid_size , threads_per_block >>> (temp2, out_GPU, num3);
        }
        else
           vector_max_kernel2 <<< grid_size , threads_per_block >>> (in_GPU, out_GPU, num3);


        break;
    case 3 :
       if(num3>256){
           vector_max_kernel3 <<< grid_size , threads_per_block >>> (in_GPU, temp, num3);
           num3 = num3 / 256;
           j=0;
           for (int i = 0; num3>256; i++){
               if (j==0){
                    vector_max_kernel3 <<< grid_size , threads_per_block >>> (temp, temp2, num3);
                    j=1;
               }
               if (j==1){
                    vector_max_kernel3 <<< grid_size , threads_per_block >>> (temp2, temp, num3);
                    j=0;
                }
                num3 = num3 / 256;
           }
           if (j==0)
                vector_max_kernel3 <<< grid_size , threads_per_block >>> (temp, out_GPU, num3);
           if (j==1)
                vector_max_kernel3 <<< grid_size , threads_per_block >>> (temp2, out_GPU, num3);
        }
        else
           vector_max_kernel3 <<< grid_size , threads_per_block >>> (in_GPU, out_GPU, num3);

        break;
    case 4 :
        //LAUNCH KERNEL FROM PROBLEM 4 HERE
       if(num3>256){
           vector_max_kernel4 <<< grid_size , threads_per_block >>> (in_GPU, temp, num3);
           num3 = num3 / 256;
           j=0;
           for (int i = 0; num3>256; i++){
               if (j==0){
                    vector_max_kernel4 <<< grid_size , threads_per_block >>> (temp, temp2, num3);
                    j=1;
               }
               if (j==1){
                    vector_max_kernel4 <<< grid_size , threads_per_block >>> (temp2, temp, num3);
                    j=0;
                }
                num3 = num3 / 256;
           }
           if (j==0)
                vector_max_kernel4 <<< grid_size , threads_per_block >>> (temp, out_GPU, num3);
           if (j==1)
                vector_max_kernel4 <<< grid_size , threads_per_block >>> (temp2, out_GPU, num3);
        }
        else
           vector_max_kernel4 <<< grid_size , threads_per_block >>> (in_GPU, out_GPU, num3);
